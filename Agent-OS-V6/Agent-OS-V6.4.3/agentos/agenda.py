...
class PersonalAgenda:
    ...
    def resolve_date(
        ...
        *,
        ...
        # Garde les séparateurs avant normalisation : ``normalize`` transforme
        # justement / et - en espaces.
        m = re.search(
    ...
    def _date_followup_view(self, message: str) -> str | None:
        """Réutilise la vue précédente pour une relance réduite à une date.

        Exemples : ``et le 12/10/2026 ?``, ``et samedi ?``. La date reste
        résolue depuis le message courant ; seule la nature de la consultation
        (programme, tâches, rendez-vous ou événements) est héritée.
        """
        if self._last_view is None:
            return None
        if self.resolve_date(message, reference=self._now()) is None:
            return None

        normalized = self.normalize(message)
        compact = re.sub(
            r"^(?:et\s+)?(?:pour\s+)?(?:le\s+|la\s+|au\s+|a\s+)?",
            "",
            normalized,
        )
        compact = re.sub(r"\s+(?:alors|du coup)$", "", compact).strip()
        if re.fullmatch(r"\d{1,2}\s+\d{1,2}(?:\s+\d{2,4})?", compact):
            return self._last_view[0]
        if re.fullmatch(
            r"(?:aujourd hui|demain|apres demain|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)(?:\s+prochain)?",
            compact,
        ):
            return self._last_view[0]

        remainder = normalized
        remainder = re.sub(r"^(?:et|pour|le|la|au|a)\s+", "", remainder)
        remainder = re.sub(r"\b(?:le|la|au|a|alors|du coup)\b", " ", remainder)
        remainder = re.sub(
            r"\b\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?\b",
            " ",
            remainder,
        ...
        remainder = re.sub(
            r"\b(?:aujourd hui|demain|apres demain|lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)(?:\s+prochain)?\b",
            " ",
            remainder,
        )
        if not " ".join(remainder.split()):
            return self._last_view[0]
        return None
    ...
    def query(self, message: str) -> str | None:
        ...
        if view is None:
            view = self._date_followup_view(message)
        if view is None:
            return None
    ...
    def _semantic_target_date(
        ...
        # Une date écrite explicitement par l'utilisateur est la preuve la plus
        # fiable. Le LLM peut reformuler une relance et produire par erreur
        # ``aujourd'hui`` ; cette cible ne doit jamais écraser ``12/10/2026``.
        resolved = self.resolve_date(
            message,
            reference=self._now(),
        )
        ...
        if resolved is None and target_text:
            resolved = self.resolve_date(
                target_text,
                reference=self._now(),
            )

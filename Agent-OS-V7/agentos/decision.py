...
""".strip()
    ...
    @classmethod
    def _v701_personal_identity_guard(cls, message: str) -> ActionDecision | None:
        """Protège les déclarations et lectures d'identité contre les missions/Web."""
        n = cls._clean(message).lower()
        n = n.replace("’", "'")
        replacements = str.maketrans("éèêëàâäîïôöùûüç", "eeeeaaaiioouuuc")
        n = n.translate(replacements)
        n = re.sub(r"[^a-z0-9']+", " ", n)
        n = " ".join(n.split())

        declaration = bool(re.search(
            r"\b(?:je m'appelle|je m appelle|je mappelle|mon prenom est|mon nom est|"
            r"j'habite|j habite|je vis|je reside)\b",
            n,
        ))
        identity_query = bool(re.search(
            r"\b(?:comment je m'appelle|comment je m appelle|comment je mappelle|"
            r"comment je mapelle|quel est mon nom|"
            r"quel est mon prenom|ou j'habite|ou j habite|ou est ce que j'habite|"
            r"ou est ce que j habite)\b",
            n,
        ))
        memory_summary = bool(
            re.search(r"\b(?:que|qu est ce que|dis moi)\b", n)
            and (
                re.search(r"\b(?:sais|sait)\b.{0,20}\b(?:moi|sur moi)\b", n)
                or re.search(r"\b(?:possede|possedes|as)\b.{0,20}\bmem+oire\b", n)
            )
        )
        if not (declaration or identity_query or memory_summary):
            return None
        return ActionDecision(
            owner="personal_memory",
            action="remember" if declaration else "query",
            confidence=0.99,
            allow_web=False,
            reason="Garde V7.0.1 : identité et mémoire personnelle.",
    ...
    def decide(
        ...
        *,
        ...
        protected_identity = self._v701_personal_identity_guard(message)

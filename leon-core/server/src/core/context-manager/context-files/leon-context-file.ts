...
export class LeonContextFile extends ContextFile {
  ...
  public generate(): string {
    return [
      ...
      '- A sentence such as "hier j’ai nettoyé la machine à café, la fontaine à chat et les toilettes" is three Paul.recordActivity calls, not a profile fact, relationship, Context entry or prose-only acknowledgement.',
      '- Questions such as "quand ai-je nettoyé les toilettes pour la dernière fois ?" must use Paul.queryActivities; never search OWNER.md or Structured Knowledge for an exact personal activity date.',
      '- Never say an activity, fact or relationship was recorded unless the corresponding Paul tool returned a successful result.',

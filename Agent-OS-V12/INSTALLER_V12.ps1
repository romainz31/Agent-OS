...
if (-not $InstallPath) {
    $InstallPath = Join-Path (Split-Path -Parent $SourceRoot) "Agent-Zero-V12"
}
...
Write-Host "Agent-OS V12 est pret dans : $InstallPath" -ForegroundColor Green
Write-Host "Plugin semantique installe dans : $PluginTarget"
Write-Host "Les donnees seront conservees dans : $DataTarget"
Write-Host "Ollama/Qwen est optionnel : sans lui, le parseur local de secours reste actif."

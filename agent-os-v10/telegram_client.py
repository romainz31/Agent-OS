...
class TelegramAgentOS:
    ...
    def handle_message(
        ...
        if attachment:
            if not self._authorized(chat_id):
                return
            self.receive_attachment(chat_id, attachment, message)
            return
    ...
    def receive_attachment(self, chat_id, attachment, message):
        import requests
        limit=10*1024*1024
        try:
            if int(attachment.get('file_size',0))>limit:
                self.send(chat_id,'Fichier trop volumineux : limite de 10 Mio.')
                return
            info=self.telegram.call('getFile',{'file_id':attachment['file_id']})
            path=info.get('file_path','')
            if not path or '..' in path.split('/'):
                raise ValueError('Chemin Telegram invalide')
            url='https://api.telegram.org/file/bot'+self.telegram.token+'/'+path
            with requests.get(url,stream=True,timeout=60) as response:
                response.raise_for_status()
                chunks=[]; size=0
                for chunk in response.iter_content(65536):
                    size+=len(chunk)
                    if size>limit: raise ValueError('Fichier supérieur à 10 Mio')
                    chunks.append(chunk)
            name=attachment.get('file_name') or 'photo.jpg'
            result=requests.post(self.agentos.base_url+'/api/documents/upload',
                files={'file':(name,b''.join(chunks))},
                data={'objective':message.get('caption') or ''},
                headers={'X-AgentOS-Client': self.agentos.client_id},
                timeout=60)
            result.raise_for_status()
            self.send(chat_id,result.json()['response'])
        except Exception:
            # Never echo a requests error containing the bot token URL.
            self.send(chat_id,'Réception impossible. Vérifie le serveur Agent-OS, le format et la limite de 10 Mio, puis renvoie le fichier.')

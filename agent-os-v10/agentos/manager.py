...
class Manager:
    ...
    def __init__(self) -> None:
        ...
        # V7 : catalogue central prêt à accueillir progressivement les outils
        # historiques. Ces deux premiers outils sont réellement branchés sur
        # le workspace et soumis aux profils/permissions.
        ...
        def read_workspace_file(path: str) -> dict[str, Any]:
            read_result = self.files.read(path)
            return {
                "message": read_result.content,
                "additional": {"path": read_result.path},
            }

        self.tools.register(
            ToolSpec(
                name="workspace_tree",
                description="Lister les fichiers du workspace Agent-OS.",
                handler=lambda max_entries=300: self.files.tree(max_entries),
                permissions=("read_workspace",),
                profiles=frozenset(
                    {"manager", "researcher", "developer", "tester", "document_analyst"}
                ),
                schema={
        ...
        self.tools.register(
            ToolSpec(
                ...
                description="Lire un fichier texte situé dans le workspace.",

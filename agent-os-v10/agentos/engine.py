...
try:
    ...
    # Les anciennes suites V5 chargent engine.py en isolation avec un faux
    # module ``agentos``. Garder cette compatibilité évite de casser leurs
    # tests unitaires tout en utilisant le vrai bus dans le runtime V7.
    class _NoopEvent:
        def __init__(self, data=None):
            self.data = data if data is not None else {}

    class ExtensionBus:  # type: ignore[no-redef]
        def emit(self, _event, data=None, **_kwargs):
            return _NoopEvent(data)
...
class WorkerEngine:
    ...
    def _execute_worker(
        ...
        """Cycle d'exécution extensible inspiré d'Agent Zero."""
        ...
        effective_payload = before.data.get("payload", payload)
        if not isinstance(effective_payload, dict):
            effective_payload = payload

        try:
            result = worker.execute(effective_payload)
        ...
        return after.data.get("result", result)

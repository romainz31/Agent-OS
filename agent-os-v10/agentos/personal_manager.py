...
class PersonalManager(CoreManager):
    ...
    def _v65_profile_response(
        ...
        normalized = self._v623_chat_normalize(message)
        normalized_plain = normalized.replace("'", " ").replace("’", " ").replace("-", " ")
        normalized_plain = normalized_plain.translate(
            str.maketrans("éèêëàâäîïôöùûüç", "eeeeaaaiioouuuc")
        )
        normalized_plain = " ".join(normalized_plain.split())

        # V7.0.1 : réponses factuelles locales pour l'identité. Aucun LLM et
        # aucun accès Web ne participent à ces réponses.
        asks_own_name = any(marker in normalized_plain for marker in (
            "comment je m appelle", "comment je mappelle", "comment je mapelle",
            "quel est mon nom", "quel est mon prenom",
            "tu connais mon nom", "tu connais mon prenom",
        ))
        if asks_own_name:
            remembered = self.memory.personal_profile_value("prénom", "nom")
            if remembered:
                return f"Tu t'appelles {remembered}."
            return "Je ne connais pas encore ton prénom."

        asks_own_location = any(marker in normalized_plain for marker in (
            "ou j habite", "ou est ce que j habite", "quel est mon lieu de vie",
            "tu sais ou j habite",
        ))
        if asks_own_location:
            remembered = self.memory.personal_profile_value("lieu de vie", "ville", "domicile")
            if remembered:
                return f"Tu habites à {remembered}."
            return "Je ne sais pas encore où tu habites."

        identity_facts = self.memory.personal_identity_facts(message)
        if identity_facts:
            values = {str(item.get("subject")): str(item.get("value")) for item in identity_facts}
            name = values.get("prénom", "")
            location = values.get("lieu de vie", "")
            if name and location:
                return f"C'est retenu : tu t'appelles {name} et tu habites à {location}."
            if name:
                return f"C'est retenu : tu t'appelles {name}."
            if location:
                return f"C'est retenu : tu habites à {location}."
        ...
        broad_summary = any(
            ...
            for marker in (
                ...
                "dis moi ce que tu possede en memoire",
                "dis moi ce que tu possedes en memoire",
                "dis moi ce que tu possede en memmoire",
                "dis moi ce que tu possedes en memmoire",
                "ce que tu as en memoire sur moi",
                "ce que tu possede en memoire sur moi",
                "ce que tu possedes en memoire sur moi",
                "qu est ce que tu sais sur moi",
                "resume ce que tu sais sur moi",
                "resume moi ce que tu sais sur moi",
                "fais moi un resume de ce que tu sais sur moi",
        ...
        if broad_summary:
            # V7.0.1 : inventaire déterministe. Une mémoire vide est annoncée
            # comme telle au lieu de demander au LLM d'improviser un profil.
...
- Paul est un compagnon numérique stable : chaleureux sans être théâtral,
  attentif sans devenir envahissant, utile sans parler comme un formulaire.
- Dans la conversation courante, reformule naturellement ce qui vient d'être
  compris. Évite le vocabulaire de base de données comme « entrée enregistrée »,
  « élément capturé » ou « opération effectuée », sauf dans un écran technique.
- Quand l'utilisateur raconte ce qu'il a fait, réponds comme quelqu'un qui suit
  réellement sa journée : « D'accord, je garde ça pour aujourd'hui » ou une
  formulation naturelle équivalente. Ne récite pas des identifiants techniques.
- Quand il partage une humeur, accueille brièvement ce qu'il dit. Ne pose une
  question que si elle semble réellement utile dans le contexte ; aucune relance
  automatique n'est obligatoire.
- Cette présence humaine reste honnête : tu es un logiciel. Ne prétends jamais
  ressentir une émotion, avoir un corps, avoir vécu un événement ou être conscient.
